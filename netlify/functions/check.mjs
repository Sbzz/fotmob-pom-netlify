...
function extractMatchId(path) { const m = path.match(/\\/match\\/(\\d{5,10})/); return m?m[1]:null; }
...
export async function handler(event) {
  try {
    let payload={};
    if(event.httpMethod==="POST"){ try{payload=JSON.parse(event.body||\"{}\");}catch{ return {statusCode:400,body:JSON.stringify({error:\"Invalid JSON\"})}; } }
    else { payload={ playerId:Number(event.queryStringParameters?.playerId||NaN), playerName:event.queryStringParameters?.playerName||\"\", matchUrl:event.queryStringParameters?.matchUrl||\"\" }; }

    const playerId=Number(payload.playerId||NaN), playerName=String(payload.playerName||\"\").trim(), matchUrl=String(payload.matchUrl||\"\").trim();
    if(!matchUrl||(!playerName&&!Number.isFinite(playerId))) return {statusCode:400,body:JSON.stringify({error:\"Need playerId/playerName + matchUrl\"})};

    const { html, finalUrl } = await fetchText(matchUrl);
    const matchId = extractMatchId(new URL(finalUrl).pathname);
    if(!matchId) return {statusCode:400,body:JSON.stringify({error:\"No matchId\"})};

    let data=null; try{ data=await fetchJSON(`https://www.fotmob.com/api/matchDetails?matchId=${matchId}`);}catch{ data=extractNextData(html); }
    if(!data) return {statusCode:500,body:JSON.stringify({error:\"No data\"})};

    const leagueId=Number(data?.general?.leagueId||data?.general?.tournamentId||null);
    ...
    const player_is_pom=potm&&(Number(potm.id)===playerId||norm(potm.name||\"\")===norm(playerName));

    return {statusCode:200,headers:{\"content-type\":\"application/json\"},body:JSON.stringify({ match_url:matchUrl, resolved_match_id:matchId, match_title:data?.general?.matchName||\"vs\", league_id:leagueId, league_label, match_datetime_utc, league_allowed, within_season_2025_26:within, player_is_pom, player_rating:null, ...stats })};
